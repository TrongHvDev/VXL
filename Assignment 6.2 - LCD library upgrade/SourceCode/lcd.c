...
void LCDWriteNibble(uint8_t ch,uint8_t rs)
{
   ...
   // LCD_PORT = (LCD_PORT | ch);
    ...
    if( (ch & 0b00000001) == 0 ) PORT_D4 = 0;
    else PORT_D4 = 1;
    if( (ch & 0b00000010) == 0 ) PORT_D5 = 0;
    else PORT_D5 = 1;
    if( (ch & 0b00000100) == 0 ) PORT_D6 = 0;
    else PORT_D6 = 1;
    if( (ch & 0b00001000) == 0 ) PORT_D7 = 0;
    else PORT_D7 = 1;
...
{
    uint8_t i=0;
    if ((_line > (NB_LINES-1)) || (_pos > (NB_COL-1)))
    {
        // Just do nothing
        return;
    }
...
void LCDPutStr3(const char *str, char _line)
{
    ...
    // LCD_Goto command
     LCDPutCmd((_line == 1) ? (0xC0 | ((16-strlen(str))/2)) : (0x80 | ((16-strlen(str))/2)));
